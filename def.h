...
struct inode {
    ...
    //Concurrent read/exclusive write var
    sem_t rw_mutex;
    int read_count;
    sem_t mutex;
};
...
//how many bytes in location
extern int bytes_track[NUM_DBLOCKS]; //global variable- tracks bytes of data blocks
extern pthread_mutex_t bytes_track_mutex; // mutex to guard mutually-exclusice access...if i complete it
...
int init_bt();// initiliaze bytes_track
void free_byte_loc(int loc);// remove anything else
